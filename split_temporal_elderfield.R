model {
  ...
  for(i in 1:length(MgCa)){
    ...
    MgCa.sw[i] ~ dnorm(MgCa.sw.m[i], 1 / MgCa.sw.sd[i] ^ 2)
    MgCa.sw.sd[i] = MgCa_sw.neo[2] + (0.3 - MgCa_sw.neo[2]) *  MgCa.age.ind[i] / (nages)
    MgCa.sw.m[i] = MgCa_sw.neo[1] + (5.2 - MgCa_sw.neo[1]) *  MgCa.age.ind[i] / (nages)
